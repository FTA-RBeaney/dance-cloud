...
const planMessage = computed(() => {
  if (auth?.data?.user?.plan === 'tier-monthly') {
    return 'Monthly';
  } else {
    return 'Yearly';
  }
...
<template>
  ...
  <div class="relative w-full h-full mt-40">
    <div
      ...
      tabindex="-1"
      aria-hidden="true"
      class="overflow-y-auto overflow-x-hidden z-50 flex justify-center items-center w-full h-full md:inset-0 h-modal md:h-full"
    >
      <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        ...
        >
          ...
          <p class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
            Successfully subscribed {{ planMessage }}.
          </p>
          <NuxtLink
            to="/courses"
            type="button"
